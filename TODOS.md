...
## Security fix wave follow-ups (filed 2026-09-15, follow-up from v0.50.2.0)

- [ ] **P2 — cwd-`.env` quarantine: decide the remaining redirect / credential env vars.**
  **What:** `src/core/env-trust.ts` quarantines a fixed list of code-loading, exec-target, root-redirect and posture-widening `GBRAIN_*` variables when the working directory's `.env` family assigns them. Left out deliberately: `GBRAIN_DATABASE_URL` (documented as the deliberate override for the #427 guard, so a hostile `.env` can still retarget the brain's database), `GBRAIN_SKILLS_DIR` / `GBRAIN_RECIPES_DIR` (content-loading path redirects), `GBRAIN_GITHUB_PAT` (credential injection), and the `serve --http` deployment trio (`GBRAIN_ADMIN_BOOTSTRAP_TOKEN`, `GBRAIN_HTTP_CORS_ORIGIN`, `GBRAIN_HTTP_TRUST_PROXY`) because container/service deployments may legitimately co-locate them. **Why:** each is the same trust-boundary class as GHSA-q5p9; the cost is a behavior change for operators who rely on project-dir `.env` files for them. **Fix:** decide per variable (quarantine, or require the `GBRAIN_`-namespaced value to also be exported), then add to `CWD_DOTENV_PROTECTED_KEYS` with a "why" comment; the structural test pins the list. **Effort:** S. **Priority:** P2.
- [ ] **P2 — the #427 `DATABASE_URL` guard matches by value and misses Bun's `${VAR}` expansion.**
  **What:** `effectiveEnvDatabaseUrl` compares `process.env.DATABASE_URL` against the literal right-hand sides parsed from the cwd `.env` files; Bun expands `${VAR}` / `$VAR` inside `.env` values before user code runs, so an assignment such as `DATABASE_URL=${DB_SCHEME}://…` produces a value the guard cannot recognise and adopts. **Why:** v0.50.2.0 switched the security-variable quarantine to key-presence semantics for exactly this reason but left the `DATABASE_URL` guard on value-match to preserve its documented "an exported, different URL still wins" behavior. **Fix:** either expand `${VAR}` references during the re-parse (mirroring Bun's rules) or move `DATABASE_URL` to key-presence with a doc note. **Effort:** S. **Priority:** P2.
- [ ] **P3 — Operator-configurable `redirect_uri` allow-list for dynamic client registration.**
  **What:** with `--enable-dcr`, a self-registered client may name any `https://` (or loopback / custom-scheme) redirect target; the owner sees it on the consent page but there is no server-side allow-list (e.g. loopback plus the browser-connector callback hosts). **Why:** suggested in a GHSA-62gp report as a cheap way to narrow the phishing payoff even with consent in place. **Fix:** `oauth.dcr_redirect_allowlist` config key checked in `registerClient` before the insert; startup summary prints the effective list. **Effort:** S. **Priority:** P3.
- [ ] **P3 — RFC 7591 initial access tokens for `/register`.**
  **What:** registration is anonymous whenever DCR is on; an operator-minted, single-use registration token would let URL-only connectors register without opening `/register` to everyone. **Why:** GHSA-42r8 suggestion; would also let `--enable-dcr` stay off by default for more deployments. **Effort:** M. **Priority:** P3.
...
- [ ] **P3 — Source fence for operator-registered `sources_admin` clients on the remaining `sources_*` mutating ops.**
  **What:** v0.50.2.0 confines remote `sources_remove` to the caller's resolved source scope (mirroring `sources_status`'s #4433 posture); `sources_archive` and the other mutating source ops still act on any id a `sources_admin` token names. **Effort:** S. **Priority:** P3.
...
- [ ] **P2 — Next security wave: the open medium/low advisories.**
  **What:** GHSA-4qc6 (admin API `POST /admin/api/api-keys` ignores `scopes` and mints full-admin keys), GHSA-pxcf (`takes_search` / `takes_list` bypass the `visibility: private` read filter), GHSA-556f (the documented `untrusted_directives` / `untrusted-quoted` controls have no implementation), GHSA-9qx8 (skill-name null-byte regex literal never matches under bun), GHSA-mjjx (`---js` front-matter: already closed by the v0.50.0.0 data-only frontmatter parser — gray-matter is no longer a dependency and every call site imports `dataFrontmatter`; the advisory just needs `patched_versions` + publish). **Why:** the v0.50.2.0 wave closed the critical/high set only. **Effort:** M. **Priority:** P2.
