...
export function createSharedSkillsAdapter(options: SharedSkillsAdapterOptions) {
  ...
  function router(snapshot: MembershipSnapshot): string {
    const route = options.launcher ? `Use only the absolute installation launcher ${JSON.stringify(options.launcher)}.`
      : `Use only the configured MCP connection ${JSON.stringify(options.connectionName ?? 'gbrain')}; never use an ambient CLI or another brain.`;
    return `---\nname: gbrain-shared-router\ndescription: Discover authorized shared brain skills relevant to the current task.\n---\n\n# Shared brain router\n\n${route}\nBrain identity: ${JSON.stringify(snapshot.brain_id)}.\nPreserve the user's identity and unrelated instructions. Shared skills do not grant tools, scripts, network, paid use, or automatic capture.\nBefore selecting a skill, call sync_brain_skills with installation_id ${JSON.stringify(snapshot.installation_id)} and enrollment_epoch ${snapshot.enrollment_epoch}. Stop shared-skill activation if that check fails; memory remains independent.\nMatch current catalog descriptions and triggers to the task. Fetch only the relevant get_skill with schema_version:2 using the complete brain/source/incarnation/pack/name key and exact revision from that authorized view. Require usable:true and delivery:complete with no unavailable requirements; never select a blocked_skills entry or infer usability from downloaded bytes. Fetch dependencies only through get_skill_asset for that same revision. Never choose an unqualified same-name skill. Never execute downloaded files automatically.\nThis paragraph is advisory, not an enforced invocation hook. Start a fresh session after updates; native activation is unverified.\n`;
  }
  async function refreshLocked(receipt: SharedSkillsLocalReceipt, admissionKey?: string): Promise<SharedSkillsLocalReceipt> {
    ...
    try {
      ...
      for (const skill of snapshot.skills) {
        const selector = { brain_id: skill.brain_id, source_id: skill.source_id, source_incarnation: skill.source_incarnation,
