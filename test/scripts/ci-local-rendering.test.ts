...
describe('required PgBouncer execution through run-e2e', () => {
  for (const [required, passes, testExit, expectedExit, parentCoverageExists, padding = 0] of [
    [true, 2, 0, 0, false],
    [true, 0, 0, 1, false],
    [true, 0, 3, 1, false],
    [false, 0, 0, 0, false],
    ...
    test(`required=${required}, executed=${passes}, Bun exit=${testExit}, parent coverage exists=${parentCoverageExists}${padding ? `, output padding=${padding}` : ''}`, () => {
...
printf ' %s pass\\n 0 fail\\n' "$FAKE_PASSES"
if [ "$FAKE_PADDING" -gt 0 ]; then printf '%*s\\n' "$FAKE_PADDING" ''; fi
exit "$FAKE_EXIT"
`, { mode: 0o755 });
        ...
        const result = spawnSync('bash', [join(home, 'scripts/run-e2e.sh'), 'test/e2e/pgbouncer-teardown.test.ts'], {
          ...
          env: {
            ...
            GBRAIN_TEST_DB: '1',
            ENV_REPORT: report, FAKE_PASSES: String(passes), FAKE_EXIT: String(testExit), FAKE_PADDING: String(padding),
          },
        });
