...
export async function sharedSkillsOAuthProcessCase(databaseUrl: string, editorPrefix = 'skills/alpha/') {
  await withTransportFixture(async f => {
    ...
    try {
      ...
      const [deniedTool] = await Promise.allSettled([call(stdio, 'get_skill', { schema_version: 2, name: 'alpha', source_id: 'hidden' })]);
      expect(deniedTool).toMatchObject({ status: 'rejected', reason: { code: 'skill_not_found' } });
      const [deniedResource] = await Promise.allSettled([stdio.readResource({ uri: forbiddenUri })]);
      expect(deniedResource).toMatchObject({ status: 'rejected', reason: { code: ErrorCode.InternalError, data: { error: 'skill_not_found' } } });
    } catch (error) {
