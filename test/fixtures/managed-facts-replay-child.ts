...
import { configureGateway, __setChatTransportForTests } from '../../src/core/ai/gateway.ts';
...
try {
  const result = await operationsByName.extract_facts.handler({ engine, config: { engine: 'postgres', embedding_disabled: true }, remote: true,
