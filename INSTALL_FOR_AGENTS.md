...
For v0.51.9.0, follow the
[mechanical shared-skills migration](skills/migrations/v0.51.9.0.md) on the host,
starting with `gbrain apply-migrations --dry-run --json`. Stop/exclude old writers
before `gbrain sources writer activate --shared-skills --confirm-quiesced`;
the flag records a real operator action, not automatic quiescence. Preserve
